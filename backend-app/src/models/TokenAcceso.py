...
class TokenAcceso:
    ...
    def save(jti : str, cedula_receptor : int) -> bool:
        ...
        with conn.cursor() as cursor:
            cursor.callproc("registrar_token_acceso", (jti, created_by, created_at, expires_at))
        return True
    ...
    def revoke(jti : str) -> bool:
        conn = Database.get_database_connection()
        with conn.cursor() as cursor:
            cursor.callproc("revocar_token_acceso", (jti,))
        return True
    ...
    def get_token_by_jti(jti : str) -> dict | None:
        ...
        conn = Database.get_database_connection()
        with conn.cursor() as cursor:
            cursor.callproc("obtener_token_acceso_por_jti", (jti,))
